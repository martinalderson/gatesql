...
if [ -n "$MCP_TOKEN" ]; then
    ...
    echo "  Claude Code:"
    echo ""
    echo "    claude mcp add --transport http gatesql http://localhost:8080/mcp \\"
    echo "      --header \"Authorization: Bearer $MCP_TOKEN\""
    echo ""
    echo "  Codex CLI:"
    echo ""
    echo "    codex --mcp-transport http --mcp-url http://localhost:8080/mcp \\"
    echo "      --mcp-header \"Authorization: Bearer $MCP_TOKEN\""
    echo ""
    echo "  Claude Desktop / other MCP clients:"
    echo ""
